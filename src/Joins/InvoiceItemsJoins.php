...
namespace BlackCat\Database\Packages\InvoiceItems\Joins;
...
final class InvoiceItemsJoins {
    ...
    /**
     * FK: invoice_items -> invoices
     * LEFT JOIN vw_invoices AS $as ON $as.tenant_id = $alias.tenant_id AND $as.id = $alias.invoice_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinInvoices(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_invoices AS ' . $as . ' ON ' . $as . '.tenant_id = ' . $alias . '.tenant_id' . ' AND ' . $as . '.id = ' . $alias . '.invoice_id' . ' ', []];
    }
