 ...
 */
final class InvoiceItemsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private InvoiceItemRepository $invoiceItemRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'invoice_items' => $this->invoiceItemRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\InvoiceItems\ModuleException("Record {$idStr} not found for locking.");
            }
