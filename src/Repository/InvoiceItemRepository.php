...
use BlackCat\Database\Packages\InvoiceItems\Repository\InvoiceItemRepositoryInterface;
...
class InvoiceItemRepository implements InvoiceItemRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'invoice_id', 'line_no' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\InvoiceItems\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'description', 'unit_price', 'quantity', 'tax_rate', 'tax_amount', 'line_total', 'currency' ]);
          return count($rows);
        }
    ...
    public function getByInvoiceIdAndLineNo(int $invoiceId, int $lineNo, bool $asDto = false): array|\BlackCat\Database\Packages\InvoiceItems\Dto\InvoiceItemDto|null {
        return $this->getByUnique([ 'invoice_id' => $invoiceId, 'line_no' => $lineNo ], $asDto);
    }
    ...
    public function getIdByInvoiceIdAndLineNo(int $invoiceId, int $lineNo) {
        $row = $this->getByInvoiceIdAndLineNo($invoiceId, $lineNo, false);
