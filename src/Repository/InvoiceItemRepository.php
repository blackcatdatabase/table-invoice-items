...
namespace BlackCat\Database\Packages\InvoiceItems\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\InvoiceItems\Definitions;
use BlackCat\Database\Packages\InvoiceItems\Criteria;
use BlackCat\Database\Packages\InvoiceItems\Dto\InvoiceItemDto as Dto;
use BlackCat\Database\Packages\InvoiceItems\Mapper\InvoiceItemDtoMapper as RowMapper;
...
final class InvoiceItemRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\InvoiceItems\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'description', 'unit_price', 'quantity', 'tax_rate', 'tax_amount', 'line_total', 'currency' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'invoice_id', 'line_no' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\InvoiceItems\Definitions::table());
            return $bulk->upsertMany($rows, [ 'invoice_id', 'line_no' ], [ 'description', 'unit_price', 'quantity', 'tax_rate', 'tax_amount', 'line_total', 'currency' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\InvoiceItems\Dto\InvoiceItemDto|null */
    public function getByInvoiceIdAndLineNo(int $invoiceId, int $lineNo, bool $asDto = false): array|\BlackCat\Database\Packages\InvoiceItems\Dto\InvoiceItemDto|null {
        $row = $this->getByUnique([ 'invoice_id' => $invoiceId, 'line_no' => $lineNo ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\InvoiceItems\Mapper\InvoiceItemDtoMapper::fromRow($row);
    }
    public function existsByInvoiceIdAndLineNo(int $invoiceId, int $lineNo): bool {
        $where = 't.' . Ident::q($this->db, 'invoice_id') . ' = :uniq_invoice_id' . ' AND ' . 't.' . Ident::q($this->db, 'line_no') . ' = :uniq_line_no';
        return $this->exists($where, [ 'uniq_invoice_id' => $invoiceId, 'uniq_line_no' => $lineNo ]);
    }
    /** @return int|string|null */
    public function getIdByInvoiceIdAndLineNo(int $invoiceId, int $lineNo) {
        $row = $this->getByInvoiceIdAndLineNo($invoiceId, $lineNo);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\InvoiceItems\Dto\InvoiceItemDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\InvoiceItems\Dto\InvoiceItemDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\InvoiceItems\Dto\InvoiceItemDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\InvoiceItems\Dto\InvoiceItemDto|null {
